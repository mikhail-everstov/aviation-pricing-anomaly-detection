...
**A regulatory surveillance framework analysing 76 million U.S. flight fares — XGBoost, Isolation Forest, SQL, and market-concentration analysis, built entirely in Snowflake.**

> Team project (Hult MBAN, Business Intelligence) — I co-developed the seven-model framework and personally built the in-Snowflake ML pipeline. Currently extending it solo with route-level booking-curve analysis (see [Extension](#extension-route-level-booking-curves-in-progress)).
...
| Finding | What we saw | Why it matters |
|---|---|---|
| **Phantom scarcity** | Flights with 3–5 seats left priced *higher* ($372) than flights with 1–2 seats left ($318) | Scarcity pricing that doesn't follow scarcity — consistent with algorithmic manipulation |
| **The nonstop paradox** | Nonstop (better product) at $251 vs one-stop (worse product) at $373 on the same corridors | 72% of the market exposed to interline pricing with no competitive discipline |
| **Refundability desert** | 1,332 refundable offers out of 82M (0.002%), at a 279% premium when shown | Consumers can't compare what they're never shown |
...
1. **What should this flight cost?** An XGBoost fair-price model (R² = 0.76) predicts each fare from 33 features. Three "justification" models then test whether demand, supply, or product factors alone can explain the price. What no model can explain becomes the *unexplained premium*.
2. **Is it structurally anomalous?** An Isolation Forest catches internally inconsistent offers, and an SQL window-function percentile ranks each fare against its direct peers (same route, same cabin — median 6,855 peers).
3. **Does the carrier have market power?** Route-level HHI (the DOJ/FTC merger standard) both scores and *amplifies* the composite — the same suspicious price matters more on a near-monopoly route.
...
Code: [`pipeline/xgboost_anomaly_snowflake.py`](pipeline/)
...
I'm rebuilding a subset of the analysis locally (the dataset is [public on Kaggle](https://www.kaggle.com/datasets/dilwong/flightprices)) to:
...
*Data: Expedia flight offers, Apr–Oct 2022 ([Kaggle: dilwong/flightprices](https://www.kaggle.com/datasets/dilwong/flightprices)). Course: DAT-8564 Business Intelligence, Hult International Business School.*
